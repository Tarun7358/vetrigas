...
export const AttendancePage: React.FC = () => {
  const { attendance, integrations, toggleIntegration } = useApp();
  const [roleFilter, setRoleFilter] = useState('ALL');

  const filtered = roleFilter === 'ALL'
    ? attendance
    : attendance.filter(a => a.role.toUpperCase() === roleFilter);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between bg-slate-900 border border-slate-800 rounded-xl p-5 text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display font-bold text-xl text-white">Biometric Attendance Registry</h1>
            <p className="text-xs text-slate-400">
              Hardware Biometric Log Direct Sync via Easy Time Pro Integration
            </p>
          </div>
        </div>
