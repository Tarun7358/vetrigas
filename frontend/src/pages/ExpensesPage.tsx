...
export const ExpensesPage: React.FC = () => {
  ...
  // PC Camera Webcam Capture State
  ...
  const startWebcam = async () => {
    try {
      ...
      alert('Could not access PC Camera / Webcam. Please check browser permissions.');
      setCamActive(false);
    }
  };
  ...
  const snapWebcamBill = () => {
    ...
    setBillImageBase64(photoBase64);
    setBillFileName(`webcam_bill_${Date.now()}.jpg`);
    closeWebcamModal();
  };
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg p-6 text-white space-y-5 shadow-2xl animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleSubmitExpense} className="space-y-4 text-xs">
              ...
              {/* Real Bill Attachment with Base64 preview & PC Camera Capture */}
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="block text-slate-400 font-semibold text-xs">Upload Bill Photo / Receipt Copy</label>
                  <button
                    type="button"
                    onClick={openWebcamModal}
                    className="text-xs bg-amber-500/20 text-amber-400 hover:bg-amber-500/30 border border-amber-500/40 px-2.5 py-1 rounded-lg font-bold flex items-center gap-1 cursor-pointer transition-colors"
                  >
                    <Camera className="w-3.5 h-3.5" /> Use PC Camera
                  </button>
                </div>
                  ...
                  {billImageBase64 ? (
                    ...
                    <div className="py-2 space-y-1">
                      ...
                      <span className="text-xs text-slate-200 block font-bold">
                        Click anywhere here to select / upload bill photo
                      </span>
                      <span className="text-[11px] text-slate-500 block">
                        Supports PNG, JPG, JPEG, WEBP photos or PC Camera capture
                      </span>
                    </div>
                  )}
                </label>
      ...
      {/* WEBCAM CAMERA MODAL FOR EXPENSE BILL SNAP */}
      {showWebcamModal && (
        <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl space-y-4 p-5 text-white">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-display font-bold text-base text-white">
                    PC Camera Bill Receipt Capture
                  </h3>
                  <p className="text-[11px] text-slate-400">
                    Hold paper bill in front of camera and snap
                  </p>
                </div>
              </div>
            ...
            <div className="relative bg-black rounded-2xl border-2 border-slate-800 overflow-hidden min-h-[300px] flex items-center justify-center">
              ...
              />
              {camActive && (
                <div className="absolute inset-4 border-2 border-dashed border-amber-400/80 rounded-2xl pointer-events-none flex items-center justify-center">
                  <span className="text-[10px] text-amber-300 font-mono bg-slate-950/80 px-3 py-1 rounded border border-amber-400/50">
                    FRAME RECEIPT HERE
                  </span>
                </div>
              )}
              {!camActive && (
                <div className="text-center py-10 space-y-2">
                  ...
                  <p className="text-xs text-slate-300">Opening PC Camera stream...</p>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-between gap-3 pt-2">
              ...
              <button
                type="button"
                onClick={snapWebcamBill}
                disabled={!camActive}
                className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-extrabold px-5 py-2.5 rounded-xl text-xs flex items-center gap-2 shadow-lg shadow-amber-500/20 cursor-pointer"
              >
